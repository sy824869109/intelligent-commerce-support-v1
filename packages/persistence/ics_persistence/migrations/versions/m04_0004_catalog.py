...
# 固定迁移快照，不导入随业务演进的运行时表定义。
metadata = sa.MetaData()
OPTIONS = dict(mysql_engine="InnoDB", mysql_charset="utf8mb4", mysql_collate="utf8mb4_bin")
# 仅用于解析外键，不创建、修改或删除身份服务的表。
sa.Table("identity_tenants", metadata, sa.Column("id", sa.String(64), primary_key=True))
...
revision = "m04_0004"
down_revision = "m03_0003"
branch_labels = None
depends_on = None


def upgrade():
    """只添加六张空读模型表，不写入演示商品或修改身份授权。"""
    metadata.create_all(op.get_bind(), tables=list(TABLES), checkfirst=False)


def downgrade():
    """任何商品或既有业务数据存在时在第一条 DDL 前拒绝降级。"""
    ...
    metadata.drop_all(op.get_bind(), tables=list(TABLES))
