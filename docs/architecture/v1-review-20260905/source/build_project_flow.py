...
def build():
    ...
    text(65, 110, "当前实施图 05  /  标准 1.1 · 2026-09-06  /  业务仍为目标设计；实施进度见任务台账", 22, GOLD, 400, "start")
    ...
    box("n3-in", 640, 725, 720, 125, "N3 · Conversation / 会话准备", ["幂等写用户消息；预留 Run / epoch；生成授权正式历史快照", "快照绑定截止消息与权限；旧草稿不进入上下文；D1 / D2"])
    ...
    box("page-shortcuts", 1470, 725, 440, 250, "页面操作 · 不经过对话编排", ["P5  N5 预检 → 明确确认 → 同域提交", "preflight_id / hash / command_id", "聊天也调用同一 N5 守卫；不造会话", "P6  知识上传与发布 → N6 / B0", "P7  工单处理与公开回复 → N7", "权限管理 → N2；结果经 N2 / N1"], PURPLE, 19)
    ...
    down(290, 1610, 1670)
    down(290, 1830, 1890)
    box("commerce-boundary", 80, 2190, 420, 275, "混合查询 · N4 顺序协调", ["① 已取得 N5 授权订单事实", "② 调 N6 / K0–K7 获取适用政策", "③ 调 N5 执行批准规则得出资格", "④ N4 汇总事实 / 依据 / 下一步", "无规则或冲突 → 不确定 / 人工", "资格不是审批；模型不执行资金"], GOLD, 19)
    down(290, 2010, 2190, "MIXED", 175)
    edge([(290, 2465), (290, 2580), (520, 2580)], arrow=False)

    box("n6-entry", 590, 1290, 530, 105, "授权包装层 → KF API → QAService", ["校验数据域 / 可见性 / 场景；绑定已发布版本", "调用原有 stream_query；不重排 KF Pipeline"], CYAN, 19)
    box("history-adapter", 1160, 1300, 250, 200, "历史适配 / C-11", ["N3 → 请求级正式快照", "K2 只读该快照", "K7 写隔离诊断历史", "诊断不回灌；撤权重建摘要", "不修改共享单例 history"], GREEN, 16)
    ...
    box("k6", 590, 2270, 530, 130, "K6 · LLM 流式生成与后处理", ["LangChain 调 D6；普通咨询受控 delta", "高风险仅进度：外层缓冲 → 检查 → 正式发布", "引用 / 置信检查不等于事实保证；见 C-09"], PURPLE, 19)
    box("k7", 590, 2450, 530, 110, "K7 · KF 内部统一收尾", ["D1：隔离诊断历史；Trace；内部 end / error", "E-11 映射子结果；N3 正式保存后才 completed"], GREEN, 19)
    ...
    box("n7", 1510, 1300, 400, 130, "人工受理 · N3 / N7", ["N4 先令 N3 HANDOFF_PENDING", "N7 按 handoff ID 建单 / 关联", "失败无假受理；旧 AI 发布被隔离"], GOLD, 18)
    ...
    box("human-pick", 1520, 1860, 380, 120, "接管握手 · N7 → N3", ["N7 唯一分配 → N3 CAS 确认 HUMAN", "确认后开放回复；丢响应按分配 ID 查"], GOLD, 17)
    box("human-write", 1520, 2040, 380, 130, "N7 保存回复与纠错", ["公开回复 + Outbox 同事务", "私密备注仅内部可见", "纠错追加保存，不覆盖原始记录"], GOLD, 18)
    box("human-event", 1520, 2230, 380, 155, "进入 B2 异步任务处理", ["N8 验来源 → N3 验公开回复", "N3 事务保存 Inbox / 消息 / Outbox", "持久成功才 ACK；同事件重投去重", "转派前合法回复可达，私密不可达"], GREEN, 17)
    ...
    box("client-end", 610, 3150, 780, 95, "正式保存成功 → N2 → N1 → 浏览器最终完成", ["普通 delta 可预览；run.completed 受落库约束；按 message_id 恢复"], GREEN, 19)
    down(1000, 2950, 2995)
    down(1000, 3105, 3150)
    box("stream-rule", 100, 2910, 420, 240, "流式与失败处理 / E-11", ["普通 delta 是预览；高风险先缓冲", "KF end 不直接变 run.completed", "N3 正式文本替换草稿，不重复追加", "命令未知 → 查询 / 对账，不重做", "取消 / 被取代答案不回灌历史"], MUTED, 19)
    ...
    box("badcase", 710, 3810, 540, 175, "B3b · Bad Case 筛选与审核", ["关联原始消息、版本、Trace 与纠错建议", "知识归 N6；路由归 N4；业务归 N5；人工归 N7", "跨领域总工作项唯一归属待 M13 ADR", "修复 → 回归 → 独立验证 → 关闭"], GOLD, 18)
...
<desc id="diagram-description">标准 1.1：正式历史适配、页面预检、混合查询、高风险缓冲、接管握手与事件映射。业务是目标设计，不代表已完成运行验收。</desc>
...
<p>当前标准 1.1 · 正式历史适配 / 两入口确认 / 混合查询 / 接管握手。业务是 V1 目标设计，93 条运行用例仍未执行。</p>
